  ...
  <div key={item.name}>
    {item.name === 'Contact' ? (
      <a href={item.link}>
        {/* Main Item */}
        <div
          className={`flex justify-between items-center p-4 rounded-md text-base font-medium cursor-pointer transition-all duration-300 ${
            isScrolled
      ...
      // Expandable items - no Link wrapper
      <>
        {/* Main Item */}
        <div
          className={`flex justify-between items-center p-4 rounded-md text-base font-medium cursor-pointer transition-all duration-300 ${
        ...
        {activeDropdown === item.name && (
          <motion.div
            className={`pl-3 mt-2 space-y-1 max-h-[300px] overflow-y-auto`}
            ...
            {item.siblings.map((sibling) => {
              ...
              return (
                <a
                  href={`/feature/${sibling.link}`}
                  key={sibling.title}
                  className={`flex flex-col items-start gap-3 p-3 rounded-md transition-all duration-300 ${
                    isScrolled
